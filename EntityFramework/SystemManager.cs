...
namespace EntityFramework
{
    ...
    public class SystemManagerOld
    {
        ...
        public void AddComponentToEntity<TComponentSystem>(Component component)
            where TComponentSystem : ComponentSystem, new()
        {
            TComponentSystem sys = this.GetComponentSystem<TComponentSystem>();

            
        }

        public void AddNewComponentToEntity<TComponent, TComponentSystem>()
            where TComponent : Component, new()
            where TComponentSystem : ComponentSystem, new()
        {
        }

        public void RemoveComponentFromEntity<TComponent, TComponentSystem>()
            where TComponent : Component, new()
            where TComponentSystem : ComponentSystem, new()
        {
        }
        ...
        public TComponentSystem GetComponentSystem<TComponentSystem>()
            where TComponentSystem : ComponentSystem, new()
        {
            this.AddComponentSystem<TComponentSystem>();
            return (TComponentSystem)this._systems[typeof(TComponentSystem).Name];
        }

        public void AddComponentSystem<TComponentSystem>(TComponentSystem com = null)
            where TComponentSystem : ComponentSystem, new()
        {
            if (!this.HasComponentSystem<TComponentSystem>())
                if (com == null)
                    this._systems.Add(typeof(TComponentSystem).Name, new TComponentSystem());
                else
                    this._systems.Add(typeof(TComponentSystem).Name, com);
        }
